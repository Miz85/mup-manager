if(Meteor.isServer){
  ...
  Meteor.methods({
    ...
    */
    ...
    "Mup.setupServer": function(config){
       var cwd = path.join(config.app, 'example');
       var oldPwd = process.cwd();


       var mupDir = path.join(oldPwd, 'npm', 'mizan_mup', 'node_modules', 'mup', 'lib');
        // process.chdir(mupDir);
